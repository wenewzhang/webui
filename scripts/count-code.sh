...
CSS_LINES=$(find ./src -type f -name "*.css" | xargs wc -l 2>/dev/null | tail -n 1 | awk '{print $1}')

echo "TypeScript (.ts) lines: ${TS_LINES:-0}"
echo "Vue (.vue) lines:     ${VUE_LINES:-0}"
echo "CSS (.css) lines:     ${CSS_LINES:-0}"
echo "Total lines:          $((${TS_LINES:-0} + ${VUE_LINES:-0} + ${CSS_LINES:-0}))"
