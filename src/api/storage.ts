...
export interface SetPoolAutoexpandResponse {
  success: boolean
  message?: string
  error: string | null
}
...
export interface DetachDeviceResponse {
  success: boolean
  message?: string
  error: string | null
}
...
export const storageApi = {
  ...
  setPoolAutoexpand(poolName: string, value: string): Promise<SetPoolAutoexpandResponse> {
    return apiClient.post('/zfs/pool_advanced_setting', { dataset: poolName, autoexpand: value }).then(res => res.data)
  },
  ...
  detachDevice(poolName: string, device: string): Promise<DetachDeviceResponse> {
    return apiClient.post('/zfs/detach_device', {
      poolname: poolName,
      device: device
    }).then(res => res.data)
  },
  attachDevice(poolName: string, device: string, newDevice: string): Promise<AttachDeviceResponse> {
    return apiClient.post('/zfs/attach_device', {
      poolname: poolName,
      device: device,
      new_device: newDevice
    }).then(res => res.data)
