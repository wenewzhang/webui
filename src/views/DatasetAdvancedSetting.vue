  ...
  <div class="pool-advanced-setting-container">
    ...
    <div v-else-if="advancedData" class="settings-content">
      <div class="pool-name-header">
        <span class="label">{{ $t('dataset.name') }}:</span>
        <span class="value">{{ datasetName }}</span>
      </div>
      
      <div class="settings-grid">
        <div v-for="(value, key) in advancedData" :key="key" class="setting-card" :class="{ 'editable': ['primarycache', 'quota', 'reservation', 'mountpoint', 'recordsize', 'atime', 'relatime', 'readonly', 'aclmode', 'acltype', 'aclinherit', 'canmount', 'logbias', 'compression', 'sync', 'checksum'].includes(key) }">
...
const fetchAdvancedSettings = async () => {
  ...
  error.value = ''
  try {
    const response = await storageApi.getPoolAdvancedSetting(datasetName.value)
    if (response.success) {
      const { autoexpand, ...rest } = response.data
      advancedData.value = rest
      // 初始化 primarycache 值
...
const savePrimaryCache = async () => {
  if (!datasetName.value || !primaryCacheValue.value) return
  ...
  try {
    const response = await storageApi.setPoolPrimaryCache(datasetName.value, primaryCacheValue.value)
    if (response.success) {
...
const saveQuota = async () => {
  ...
  try {
    const response = await storageApi.setPoolQuota(datasetName.value, finalQuota)
    if (response.success) {
...
const saveReservation = async () => {
  ...
  try {
    const response = await storageApi.setPoolReservation(datasetName.value, finalReservation)
    if (response.success) {
...
const saveMountpoint = async () => {
  ...
  try {
    const response = await storageApi.setPoolMountpoint(datasetName.value, trimmedValue)
    if (response.success) {
...
const saveRecordsize = async () => {
  if (!datasetName.value || !recordsizeValue.value) return
  ...
  try {
    const response = await storageApi.setPoolRecordsize(datasetName.value, recordsizeValue.value)
    if (response.success) {
...
const saveAtime = async () => {
  if (!datasetName.value || !atimeValue.value) return
  ...
  try {
    const response = await storageApi.setPoolAtime(datasetName.value, atimeValue.value)
    if (response.success) {
...
const saveRelatime = async () => {
  if (!datasetName.value || !relatimeValue.value) return
  ...
  try {
    const response = await storageApi.setPoolRelatime(datasetName.value, relatimeValue.value)
    if (response.success) {
...
const saveReadonly = async () => {
  if (!datasetName.value || !readonlyValue.value) return
  ...
  try {
    const response = await storageApi.setPoolReadonly(datasetName.value, readonlyValue.value)
    if (response.success) {
...
const saveAclmode = async () => {
  if (!datasetName.value || !aclmodeValue.value) return
  ...
  try {
    const response = await storageApi.setPoolAclmode(datasetName.value, aclmodeValue.value)
    if (response.success) {
...
const saveAcltype = async () => {
  if (!datasetName.value || !acltypeValue.value) return
  ...
  try {
    const response = await storageApi.setPoolAcltype(datasetName.value, acltypeValue.value)
    if (response.success) {
...
const saveAclinherit = async () => {
  if (!datasetName.value || !aclinheritValue.value) return
  ...
  try {
    const response = await storageApi.setPoolAclinherit(datasetName.value, aclinheritValue.value)
    if (response.success) {
...
const saveCanmount = async () => {
  if (!datasetName.value || !canmountValue.value) return
  ...
  try {
    const response = await storageApi.setPoolCanmount(datasetName.value, canmountValue.value)
    if (response.success) {
...
const saveLogbias = async () => {
  if (!datasetName.value || !logbiasValue.value) return
  ...
  try {
    const response = await storageApi.setPoolLogbias(datasetName.value, logbiasValue.value)
    if (response.success) {
...
const saveCompression = async () => {
  if (!datasetName.value || !compressionValue.value) return
  ...
  try {
    const response = await storageApi.setPoolCompression(datasetName.value, compressionValue.value)
    if (response.success) {
...
const saveSync = async () => {
  if (!datasetName.value || !syncValue.value) return
  ...
  try {
    const response = await storageApi.setPoolSync(datasetName.value, syncValue.value)
    if (response.success) {
...
const saveChecksum = async () => {
  if (!datasetName.value || !checksumValue.value) return
  ...
  try {
    const response = await storageApi.setPoolChecksum(datasetName.value, checksumValue.value)
    if (response.success) {
...
// 返回上一页
const goBack = () => {
  router.push('/storage/datasets')
}

onMounted(() => {
  const name = route.query.dataset as string
  if (name) {
    datasetName.value = name
    fetchAdvancedSettings()
  } else {
    error.value = 'No dataset name provided'
  }
})
</script>
