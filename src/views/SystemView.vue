<template>
  <div class="fixed inset-0 z-50 bg-black/60 flex items-center justify-center">
    <div class="bg-gray-900 border border-gray-700 shadow-2xl rounded-xl p-10 max-w-md w-full text-center">
      ...
      <label v-if="isRestart" class="flex items-center justify-center space-x-2 mb-8 cursor-pointer select-none">
        <input
          v-model="forceRestart"
          type="checkbox"
          class="h-4 w-4 rounded border-gray-600 bg-gray-800 text-indigo-600 focus:ring-indigo-500"
        />
        <span class="text-sm text-gray-300">{{ $t('system.forceRestarting') }}</span>
      </label>
...
const handleAction = async () => {
  ...
  try {
    ...
    if (isRestart.value) {
        ...
        } else  res = await systemApi.reboot()
    } else {
